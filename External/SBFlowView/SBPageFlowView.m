...
- (void)refreshVisibleCellAppearance2
{
    if (_minimumPageAlpha == 1.0 && _minimumPageScale == 1.0) {
        return;//无需更新
    }
...
- (void)reloadDataForScrollView:(PageDirectionE)direction
{
    if (direction == PageDirectionPrevious) {
        ...
        reuseCell.transform = CGAffineTransformMakeScale(0.0, 0.0);
        reuseCell.alpha = 0.0;
        [UIView animateWithDuration:0.9
                         animations:^{
                             reuseCell.alpha = 1.0;
                             reuseCell.transform = CGAffineTransformIdentity;
                         }
...
- (void)scrollToNextPage
{
    ...
    if (newItem == nil) {
        UIView  *cell1 = [_dataSource flowView:self cellForPageAtIndex:0];
        cell1.tag = 0 + kTagOffset;
    ...
    [UIView animateWithDuration:0.5f
                     animations:^{
                         for (NSInteger  i=0; i<[_inUseCells count]; i++) {
                             UIView  *v = [_inUseCells objectAtIndex:i];
                             v.frame = CGRectMake(_pageSize.width*(i-1), 0, _pageSize.width, _pageSize.height);
                             NSLog(@"%@ d ddddd", NSStringFromCGRect(v.frame));
                             
                         }
                         [_inUseCells removeObjectAtIndex:0];
                         [self refreshVisibleCellAppearance2];
                         ...
                         NSLog(@"nextPage index %d", index);
                         
                         reuseCell.tag = index + kTagOffset;
                         reuseCell.frame = CGRectMake(_pageSize.width*2, 0, _pageSize.width, _pageSize.height);
                         NSLog(@"%@", NSStringFromCGRect(reuseCell.frame));
                         
                         for (NSInteger  i=0; i<[_inUseCells count]; i++) {
                             UIView  *v = [_inUseCells objectAtIndex:i];
                             NSLog(@"_inUseCells %@", NSStringFromCGRect(v.frame));
                             
                         }
                         
                         for (NSInteger  i=0; i<[_reusableCells count]; i++) {
                             UIView  *v = [_reusableCells objectAtIndex:i];
                             NSLog(@"_reusableCells %@", NSStringFromCGRect(v.frame));
    ...
    [UIView animateWithDuration:0.5f
                     animations:^{
                         for (NSInteger  i=0; i<[_inUseCells count]; i++) {
                             UIView  *v = [_inUseCells objectAtIndex:i];
                             v.frame = CGRectMake(_pageSize.width*(i-1), 0, _pageSize.width, _pageSize.height);
                             NSLog(@"_inUseCells dddd  %@", NSStringFromCGRect(v.frame));
                         }
                         [_inUseCells removeLastObject];
                         [self refreshVisibleCellAppearance2];
